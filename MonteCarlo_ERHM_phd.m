    ...
    addpath('_common');
    model = gen_ERHM_model();                                               %load model
    filter = gen_ERHM_filters();                                            %load filter
    rng(2022); 
    montecarlo = 200;
...
%sccenario2
    ...
    %% output variables
    for i=1:montecarlo
        outcome{i}.output_ERHM_phd.state_X_hat = cell(length(meas),1);                          % inferred target states
        outcome{i}.output_ERHM_phd.state_E_hat = cell(length(meas),1);
        outcome{i}.output_ERHM_phd.state_n_hat = zeros(length(meas),1);                         % estimated number of targets
        outcome{i}.output_ERHM_phd.filter = filter;                                       % store filtering params
        outcome{i}.output_ERHM_phd.state_n_hat(1) = 2;
    end
    %% 初始化分布
    clear w a b m P nu V J                                                  % 用于ERHM_PHD初始化
    ...
    for i=1:montecarlo
        ...
        [w,a,b,m,P,J] = ERHM_reduction(w,a,b,m,P,J,filter);
            ...
            %状态提取
            [~,outcome{i}.output_ERHM_phd.state_X_hat{k},~,outcome{i}.output_ERHM_phd.state_E_hat{k}] = ...
                ERHM_stateExtraction(w,a,b,m,P,J,model);
            outcome{i}.output_ERHM_phd.state_n_hat(k) = size(outcome{i}.output_ERHM_phd.state_X_hat{k},2);
        end
    end
    %% return
    ...
    for i=1:montecarlo
        ...
        for k = 1:length(truth)
            error_t(k) = ospa_dist( get_comps(truth{k}.position,[1,2]), get_comps(outcome{i}.output_ERHM_phd.state_X_hat{k},[1 2]), ...
                model.ospa_cutoff, model.ospa_order);
            ave_ospa(k) = ave_ospa(k) + error_t(k);
        end
        outcome{i}.output_ERHM_phd.ospa = error_t;
    end
